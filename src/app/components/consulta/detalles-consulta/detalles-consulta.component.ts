...
export class DetallesConsultaComponent implements OnInit {
  ...
  constructor(
    ...
    private matRef:MatDialogRef<DetallesConsultaComponent>
  ) { }

  ngOnInit() {
    this.matRef.updateSize('100%','90%');
  }
  ...
  convertToBase64(file: any, callback?: any) {
    ...
    reader.onload = (event: any) => {
      this.filtrosF.value.pagoImssFile = event.target.result;
    };
  ...
  handlePagoImssFileInput(event: any) {
    ...
    this.filtrosF.value.pagoImssFile = event.target.files[0];
